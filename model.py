...
def read_data():
    ...
    indices = GroupShuffleSplit(n_splits=1,train_size=0.7,random_state=42)
    ...
    validation_indices = torch.utils.data.SubsetRandomSampler(validation)
    batch_sample_train = torch.utils.data.BatchSampler(train_indices,batch_size=32,drop_last=False)
    batch_sample_val  = torch.utils.data.BatchSampler(validation_indices,batch_size=32,drop_last=False)
    train_data = torch.utils.data.DataLoader(data,batch_sampler=batch_sample_train)
    test_data  = torch.utils.data.DataLoader(data,batch_sampler=batch_sample_val)
...
def main():
    ...
    train_data_loader,val_data_loader =  read_data()
    iteration = 1
    prev_val_loss= 0.0
    prev_train_loss =0.0
...
if __name__ == "__main__":
    ...
    if len(os.listdir(weight_folder)) == 0:
        main()
    else:
        with torch.no_grad():
            _,test= read_data()
